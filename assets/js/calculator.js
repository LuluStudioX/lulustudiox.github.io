document.addEventListener('DOMContentLoaded', function () {
  ...
  if (savedTrainingSpeed)
    document.getElementById('trainingSpeed').value = savedTrainingSpeed;
  if (savedTrainingCapacity)
    document.getElementById('trainingCapacity').value = savedTrainingCapacity;
  if (savedWantedTroops)
    document.getElementById('wantedTroops').value = savedWantedTroops;
  ...
  Promise.all([
    ...
    .then(([baseTimeData, infantryData, lancerData, marksmenData]) => {
      ...
      form.addEventListener('submit', function (e) {
        ...
        const trainingCapacity = parseFloat(
        ...
        const wantedTroops = parseInt(
          document.getElementById('wantedTroops').value
        );
        ...
        const baseTime = baseTimeData.find(
          (tier) => tier.tier === troopTier
        ).time;

        // Calculate total training time
        const totalTrainingSeconds =
          baseTime * trainingMultiplier * wantedTroops;
        ...
        if (troopData) {
          const resourceCost = troopData.find(
            (t) => t.tier === troopTier
          ).resource_cost;

          rawResourceValues = {
            meat: resourceCost.meat * wantedTroops,
            wood: resourceCost.wood * wantedTroops,
            coal: resourceCost.coal * wantedTroops,
            iron: resourceCost.iron * wantedTroops,
          };

          // Calculate number of sessions needed
          const sessionsNeeded = Math.ceil(wantedTroops / maxCapacity);
          ...
          if (capacityBuffPercentage > 0 || flatCapacityIncrease > 0) {
            ...
            buffInfo += `Your effective Training Capacity is <strong><i><span data-tooltip="${capacityTooltip}">${Math.floor(
          ...
          results.innerHTML = `
                    Training <strong><i>${wantedTroops}</i></strong> 
                    <strong>${
                      troopType.charAt(0).toUpperCase() + troopType.slice(1)
                    }</strong> 
                    ...
                    It will take <strong><i>${sessionsNeeded}</i></strong> Sessions and costs:<br>
                        🥩 ${formatNumber(
                          rawResourceValues.meat
                        )} | 🪵 ${formatNumber(rawResourceValues.wood)} | 
                        ⚒️ ${formatNumber(
                          rawResourceValues.coal
                        )} | 🪨 ${formatNumber(rawResourceValues.iron)}<br><br>
                        ${buffInfo}
                    `;
      ...
      submitBuffsButton.addEventListener('click', function () {
        ...
        document
          .querySelectorAll('.buff-option input[type=checkbox]')
          .forEach((checkbox) => {
            if (checkbox.checked) {
              if (checkbox.id === 'customBuff') {
                const speedBuff =
                  document.getElementById('customSpeedBuff').value;
                const capacityBuff =
                  document.getElementById('customCapacityBuff').value;
                selectedBuffs.push(
                  `Custom Buff (${speedBuff}% speed, +${capacityBuff} capacity)`
                );
              } else {
      ...
      function updateSelectedBuffDisplay() {
        if (selectedBuffs.length > 0) {
          toggleBuffsButton.textContent = `${
            selectedBuffs.length
          } Buff(s) Applied: ${selectedBuffs.join(', ')}`;
        } else {
          toggleBuffsButton.textContent = `Select Buffs`;
        }
      }
      ...
      document
        .getElementById('customBuff')
        .addEventListener('change', function () {
          const customBuffInputs = document.querySelector(
            '.custom-buff-inputs'
          );
  ...
  function setupMutuallyExclusiveBuffs() {
    ...
    const ministerOfEducationCheckbox = document.getElementById(
      'ministerOfEducation'
    );
